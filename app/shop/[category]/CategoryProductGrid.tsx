...
export function CategoryProductGrid({
  ...
  const OptionRow = ({
    ...
    >
      ...
      {typeof count === 'number' && (
        <span className={`font-mono text-[11px] ${active ? 'text-white/80' : 'text-stone-400'}`}>{count}</span>
      )}
    </button>
  );
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            <svg className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
